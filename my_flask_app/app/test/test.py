...
def generate_video_test(narration, youtube_link):
    try:
        download_video_from_youtube_test(youtube_link)
        text_to_speech_test(narration)
...
def generate_video_test():
    ...
    generate_video_test(narration_text, youtube_link)

def download_video_from_youtube_test(url):
    try:
        yt = YouTube(url)
